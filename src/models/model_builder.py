...
class Bert(nn.Module):
    def __init__(self, large, temp_dir, finetune=False,other=None):
        super(Bert, self).__init__()
        if (large):
            self.model = BertModel.from_pretrained('/home/NLP/pre-training-model/bert-large')
        else:
        # 用其他模型的话
            if other is not None:
                self.model = RobertaModel.from_pretrained('/storage/longjing/roberta-base')
            else:
                self.model = BertModel.from_pretrained('/storage/longjing/bert-base')
        # 是否保留梯度
    ...
    def forward(self, x, segs=None, mask=None):
        if self.finetune:
            ...
            top_vec = self.model(x, token_type_ids=segs).last_hidden_state
        else:
            self.eval()
            with torch.no_grad():
                top_vec, _ = self.model(x, segs, attention_mask=mask)
...
class ExtSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint):
        ...
        self.args = args
        self.device = device
        self.bert = Bert(args.large, args.temp_dir, args.finetune_bert, other=args.encoder)
    ...
    def dim_function(self, top_vec, mask_src, clss, mask_cls):
        sents_vec = None
        for corpus, attmask, flag in zip(top_vec, mask_src, clss):
            ...
            a = torch.nonzero(attmask == True)
            # 获取整句的句长
            max_sent_len = int(a[-1]+1) if a.shape[0] != 0 else int(top_vec.shape[1])
    ...
    def forward(self, src, segs, clss, mask_src, mask_cls):
        ...
        # 送进dimAttention
        sents_vec = self.dim_function(top_vec, mask_src, clss, mask_cls)
        # 对多余的句子padding进行掩码
...
def get_sent_vet(top_vec, mask_src, clss):
    ...
    for corpus, attmask, flag in zip(top_vec, mask_src, clss):
        ...
        for i in range(len(flag) - 1):
            ...
            b = flag[i + 1]
            # 判断这条语料是否含有字符padding
            c = torch.nonzero(attmask == True)
            # 有padding的话
            if c.shape[0] != 0:
                max_sent_len = int(c[-1]+1)
            # 没有padding的话
            else:
